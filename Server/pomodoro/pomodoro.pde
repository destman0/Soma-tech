...
long pomodoro_duration, rest_duration;
...
public void HRV_Interaction_Async() {
  if (selection != SelectedInteraction.NotReady && selection != SelectedInteraction.SlowBreathingAsync) {
    onInteractionChanged(SelectedInteraction.SlowBreathingAsync);
  }
}
...
public void Inflate_All_Pillows() {
  if (selection != SelectedInteraction.NotReady && selection != SelectedInteraction.InflateAll) {
    onInteractionChanged(SelectedInteraction.InflateAll);
  }
}
...
void interaction_Five() {
  ...
  if (interaction_part==1) {
    ...
    if ((interactioncurrenttime - longinteractionstarttime)<(slow_breathing_duration*60000)) {
      ...
      OscMessage myMessage1, myMessage2;
      myMessage1 = new OscMessage("/actuator/inflate");
      ...
      case 0:
        //println("Inhale");
        myMessage1.add(cp5.getController("Inflation_Rate").getValue());
        myMessage2.add(-(cp5.getController("Deflation_Rate").getValue()));
      ...
      case 1:
        //println("Exhale");
        myMessage1.add(-(cp5.getController("Deflation_Rate").getValue()));
        myMessage2.add(cp5.getController("Inflation_Rate").getValue());
        sendToOneActuator(myMessage1, 1);
        sendToOneActuator(myMessage2, 2);
        //myTextarea2.setText("HOLD "+(interactioncurrenttime-(phase*phasedur+interactionstarttime))/1000);
        break;
