...
Open `resp_pace_adaptive.pde` in Processing Java mode. Keep `BreathingUI.pde` and `BreathSession.java` alongside it. Requires the existing oscP5, ControlP5, Sound and SignalFilter libraries. Original source is preserved in `original-resp-pace.pde.txt`.
