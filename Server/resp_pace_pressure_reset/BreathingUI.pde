// UI and original OSC server adapter. Both pillows share device 1.
BreathSession breath=new BreathSession();
int PRESSURE_DEVICE_ID=1;
...
void drawBreathingUI(){
  ...
  centered("Illustrative movement / both pressure displays use device 1",410,797,12,muted);
...
void mousePressed(){
  ...
  for(int i=0;i<5;i++){float y=381+i*75;if(hit(831,y-18,375,36)){focusSetting=i;if(mouseX<866)changeSetting(i,breath.values[i]-breath.step[i]);else if(mouseX>1174)changeSetting(i,breath.values[i]+breath.step[i]);else changeSetting(i,map(mouseX,874,1162,(float)breath.minimum[i],(float)breath.maximum[i]));return;}}
}
void mouseDragged(){int i=focusSetting;if(i>=0&&hit(866,363+i*75,308,36))changeSetting(i,map(mouseX,874,1162,(float)breath.minimum[i],(float)breath.maximum[i]));}
