...
Open **pomodoro_device3_restart.pde** in Processing (Java mode) and Run. Keep SoleUI.pde in the same folder. Uses the existing ControlP5, oscP5, Sound and SignalFilter libraries. The original source is preserved in original-pomodoro.pde.txt.
...
- Restart during inflation or focus (including paused): deflates for the current Deflate for duration, using Deflation strength, then starts a fresh 10-second preparation period and Pomodoro. This is timed deflation, not confirmation of empty pads. In other stages, Restart begins preparation immediately as before. Pause/Resume works during restart deflation; Stop cancels the pending restart. Repeated Restart clicks during restart deflation do not skip or extend it. Manual controls cancel the pending restart.
...
SoleUI.pde contains the new visual layer and interaction handlers. pomodoro_device3_restart.pde retains the original server and timing logic, with hidden legacy widgets, pause handling, pressure timestamps and a corrected end-of-session boundary. The main filename now matches the sketch folder as required by Processing.
