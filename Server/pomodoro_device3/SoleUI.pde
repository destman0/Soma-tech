...
int uiPhase(){return !uiRunning()?-1:interaction_part==0?0:phase+1;}
long uiDuration(){
  int p=uiPhase();
...
void uiStop(){uiPaused=false;selection=SelectedInteraction.StopAll;interactionstarted=false;interaction_part=0;stopping_Units();uiNotice="Stopped. Pumps are off; pads are not actively deflating.";}
...
void uiStart(){uiPaused=false;interaction_part=0;phase=0;interactionstarted=false;selection=SelectedInteraction.Pomodoro;stopping_Units();uiNotice="10 seconds to settle in, then inflate / focus / deflate / break.";}
void uiToggle(){
...
void mousePressed(){
  ...
  if(uiHit(346,459,130,46)){uiStart();return;}
...
void keyPressed(){
  ...
  else if(key==' ')uiToggle();else if(key=='s'||key=='S')uiStop();else if(key=='r'||key=='R')uiStart();
